...
function library:new(props)
	...
	local outline4 = utility.new(
		"ImageLabel",
		{
			BackgroundColor3 = Color3.fromRGB(0, 0, 0),
			BackgroundTransparency = 1,
			Image = (getgenv().image or "rbxassetid://106797409902358"),
			Size = UDim2.new(1,0,1,0),
...
function library:saveconfig()
    ...
    for i, v in pairs(self.pointers) do
        ...
        for c, d in pairs(v) do
            -- ✅ Handle direct control (flat) vs. nested section
            if d.current ~= nil then
                -- flat pointer (e.g., toggle directly under a section)
...
function library:loadconfig(path)
    ...
    for pageName, pageData in pairs(decoded) do

        local pagePointer = self.pointers[tostring(pageName)]
        if not pagePointer then
            continue
        end

        for sectionName, sectionData in pairs(pageData) do

            local sectionPointer = pagePointer[tostring(sectionName)]
            if not sectionPointer then
                continue
            end

            for featureName, value in pairs(sectionData) do

                local feature = sectionPointer[tostring(featureName)]
                if not feature then
                else
                    if feature.set then
    
                        feature:set(value)
                    elseif feature.current ~= nil then
                        feature.current = value
                        if feature.callback then
                            pcall(feature.callback, feature.current)
                        end
                    end
                end
            end
...
function library:page(props)
	...
	--
	local pointer = props.pointer or props.Pointer or props.pointername or props.Pointername or props.PointerName or props.pointerName or props.name or props.Name or nil

	--
if pointer then
    -- make sure pointers table exists
    self.pointers = self.pointers or {}

    -- store page by string name, not by table reference
...
function pages:section(props)
	...
	--
if pointer then
    -- ensure both the page and section have pointer tables
    self.pointers = self.pointers or {}
    section.pointers = section.pointers or {}

    -- register this section under its page
    self.pointers[tostring(pointer)] = section.pointers

    -- also ensure this page is registered in the library hierarchy
    self.library.pointers = self.library.pointers or {}
    self.library.pointers[self.name or tostring(self)] = self.pointers
end
...
function pages:multisection(props)
	...
	--
if pointer then
    -- ensure both the page and multisection have pointer tables
    self.pointers = self.pointers or {}
    multisection.pointers = multisection.pointers or {}

    -- register this multisection under its page
...
function multisections:section(props)
	...
	--
if pointer then
    -- ensure both this multisection and sub-section have pointer tables
    self.pointers = self.pointers or {}
    mssection.pointers = mssection.pointers or {}

    -- register this sub-section under its parent multisection
    self.pointers[tostring(pointer)] = mssection.pointers

    -- also ensure this multisection is registered inside the library hierarchy
    self.library.pointers = self.library.pointers or {}
    self.library.pointers[self.name or tostring(self)] = self.pointers
end
...
function sections:toggle(props)
	...
	--
if pointer then
    -- ensure the section has its own pointer table
    self.pointers = self.pointers or {}

    -- register this toggle under the section
    self.pointers[tostring(pointer)] = toggle

    -- ensure the library has a proper nested structure
    self.library.pointers = self.library.pointers or {}

    -- find this section’s parent page name if possible
    local pageName = self.page and self.page.name or self.library.currentpage
    local sectionName = self.name or tostring(self)

    if pageName then
        -- make sure the page table exists in the hierarchy
        self.library.pointers[pageName] = self.library.pointers[pageName] or {}
        -- link this section’s pointers under its page
        self.library.pointers[pageName][sectionName] = self.pointers
    else
        -- fallback if we can’t detect a page (rare edge case)
        self.library.pointers[sectionName] = self.pointers
    end
end
...
function sections:slider(props)
	...
	--
if pointer then
    -- ensure this section has a pointer table
    self.pointers = self.pointers or {}

    -- register this slider under its section
    self.pointers[tostring(pointer)] = slider

    -- also ensure this section is registered up the chain
    self.library.pointers = self.library.pointers or {}
    self.library.pointers[self.name or tostring(self)] = self.pointers
end
...
if pointer then
    ...
    -- register this dropdown under its section
    self.pointers[tostring(pointer)] = dropdown

    -- also ensure this section is registered up the hierarchy
    self.library.pointers = self.library.pointers or {}
    self.library.pointers[self.name or tostring(self)] = self.pointers
end
...
if pointer then
    ...
    -- register this buttonbox under its section
    self.pointers[tostring(pointer)] = buttonbox

    -- also ensure this section is registered in the library hierarchy
    self.library.pointers = self.library.pointers or {}
    self.library.pointers[self.name or tostring(self)] = self.pointers
end
...
if pointer then
    ...
    -- register this multibox under its section
...
if pointer then
    ...
    -- register this textbox under its section
...
function sections:keybind(props)
	...
	--
-- // auto-register keybind safely
local lib = self.library
lib.pointers = lib.pointers or {}

local tab = tostring(self.tabname or "DefaultTab")
local section = tostring(self.sectionname or "DefaultSection")

lib.pointers[tab] = lib.pointers[tab] or {}
lib.pointers[tab][section] = lib.pointers[tab][section] or {}
lib.pointers[tab][section][tostring(name)] = keybind

-- also keep a local pointer reference (optional, for backwards compatibility)
self.pointers = self.pointers or {}
self.pointers[tostring(name)] = keybind
...
if pointer then
    ...
    -- register this colorpicker under its section
...
createdbutton.MouseButton1Down:Connect(function()
	...
	createdb.title.TextColor3 = self.library.theme.accent
	table.insert(self.library.themeitems["accent"]["TextColor3"],createdb.title)
	currentname = createdb.name -- ✅ FIXED: store string name, not UI table
end)
    ...
    -- ✅ Save current selection before wiping buttons
    local savedCurrent = currentname
    ...
    -- Recreate config buttons
    for i, v in pairs(listfiles(folder)) do
        if v:sub(-4) == ".cfg" then
            local name = v:sub(#tostring(folder)+2, -5)
            name = name:gsub("%s+$", "")
            ...
            -- ✅ Restore selection highlight if matches the previously chosen config
            if name == savedCurrent then
                currentname = savedCurrent
            end
        end
    end
end
...
name[2].FocusLost:Connect(function()
    -- ✅ Only ignore reset if a config is selected AND the textbox is empty.
    if selected and name[2].Text == "" then return end
...
load[3].MouseButton1Down:Connect(function()
    local toload = currentname or (name[2].Text ~= "" and name[2].Text) or nil
    if toload then
        local cfgpath = folder.."/"..toload..".cfg"
        if isfile(cfgpath) then
            self.library:loadconfig(cfgpath)
        end
    end
	...
	delete[3].MouseButton1Down:Connect(function()
		if currentname then
			local cfgpath = folder.."/"..currentname..".cfg"
			if isfile(cfgpath) then
				delfile(cfgpath)
			end
			refresh()
	...
	save[3].MouseButton1Down:Connect(function()
		if currentname then
			local cfgpath = folder.."/"..currentname..".cfg"
			writefile(cfgpath, self.library:saveconfig())
			refresh()
		end

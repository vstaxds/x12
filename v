...
function library:loadconfig(path)
	...
	if not isfile(path) then
		warn("[Debug] File does not exist")
		return
	end
	...
	if not ok1 then
		warn("[Debug] readfile failed:", raw)
		return
	end
	print("[Debug] readfile returned:", string.sub(raw, 1, 200)) -- only print first 200 chars
	...
	if not ok2 then
		warn("[Debug] JSONDecode failed:", data)
		return
	end
	print("[Debug] JSONDecode succeeded, type:", typeof(data))
...
print("[Debug] Setting:", i, c, x, typeof(self.pointers[i][c][x]), typeof(z))
self.pointers[i][c][x]:set(z)
	...
	print("[Debug] Finished loadconfig successfully.")
end

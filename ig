...
function library:loadconfig(path)
	...
	local ok1, raw = pcall(readfile, path)
	if not ok1 then
		return
	end
	...
	for i, v in pairs(data) do
		for c, d in pairs(v) do
			for x, z in pairs(d) do
				if z ~= nil then
					local control = self.pointers[i]
						and self.pointers[i][c]
						and self.pointers[i][c][x]

					if control then
						control:set(z)

						-- ✅ Trigger callback for active controls on load
						if control.callback and (
							control.current == true
							or typeof(control.current) == "number"
							or typeof(control.current) == "Color3"
						) then
							pcall(control.callback, control.current)
						end
					end
				end
			end
...
local refresh = function()
	...
	for i,v in pairs(listfiles(folder)) do
		if v:sub(-4) == ".cfg" then
			...
			name = name:gsub("%s+$", "")  -- ✅ trim trailing spaces from config name
			if i == 1 then 
